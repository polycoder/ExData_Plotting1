dev.off() # close device to reset any lingering par settings

# load polycoder_pwr_data table if it does not exist in the environment.
if(!exists("polycoder_pwr_data")) {
  script.dir <- dirname(sys.frame(1)$ofile)
  polycoder_pwr_data <- read.csv(
    paste(script.dir, "household_power_consumption.txt", sep="/"), 
    sep=";",
    dec=".", 
    na.strings="?",
    header=TRUE,
